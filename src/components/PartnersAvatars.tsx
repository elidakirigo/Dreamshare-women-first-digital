      ...
      <div className="relative mt-4">
        <Avatar className="h-[9rem] w-[9rem] rounded-full shadow-md">
          {results.map(({ picture }: { picture: { large: string } }, i) => (
            <AvatarImage
              key={i}
              alt="user avatars"
              src={picture.large || "/assets/formatted/user-profile.webp"}
            />
          ))}
        ...
        {svg && (
          <Image
            ...
            height={40}
            alt="avatar icons"
            className={`z-1 absolute bottom-0 right-2 rounded-full border-4 border-white bg-[#661F20] p-2 shadow-md`}
          />
        )}
      </div>
